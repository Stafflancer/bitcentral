...
add_shortcode('FUEL-TilesByCategory', 'fuel_bitcentral_tiles_by_category');

function fuel_bitcentral_tiles_by_category($data) {

    $content = '<div class="fuel-related-stories tiles-by-category">
    ...
    if (isset($data['category']) && isset($data['max']) && !empty($data['category']) && !empty($data['max'])) {
        ...
        if ($query_related->have_posts()) {
            ...
            $content .= fuel_bitcentral_tiles_ul($query_related, 'false', $cat_name, $shorcode_options);
